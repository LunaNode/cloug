...
func (api *API) ListServiceOfferings() ([]ServiceOffering, error) {
	var response ListServiceOfferingsResponse
...
func (api *API) ListDiskOfferings() ([]DiskOffering, error) {
	var response ListDiskOfferingsResponse
...
func (api *API) DeployVirtualMachine(options *DeployVirtualMachineOptions) (string, string, error) {
	params := map[string]string{
		"serviceofferingid": options.ServiceOffering,
		"diskofferingid":    options.DiskOffering,
		"templateid":        options.Template,
	}
	if options.Network != "" {
		params["networkids"] = options.Network
	}
	if options.Name != "" {
		params["name"] = options.Name
	}

	var response DeployVirtualMachineResponse
...
func (api *API) QueryDeployJob(jobid string) (*DeployVirtualMachineResult, error) {
	type JobResult struct {
		VirtualMachine *DeployVirtualMachineResult `json:"virtualmachine"`
	}
...
func (api *API) GetVirtualMachine(id string) (*VirtualMachine, error) {
	params := map[string]string{"id": id}
	var response ListVirtualMachinesResponse
...
func (api *API) ListVirtualMachines() ([]VirtualMachine, error) {
	var response ListVirtualMachinesResponse
	err := api.request("listVirtualMachines", nil, &response)
	if err != nil {
		return nil, err
	} else {
